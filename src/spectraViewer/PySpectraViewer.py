...
class  SpectraViewMain(QtWidgets.QMainWindow, Ui_MainWindow):
	...
	def __init__(self, parent=None):
		...
		self.EntryButton.clicked.connect(self.to_logBook)
		...
		self.shutter_signal.connect(self.shutter_set)
		self.trigger_signal.connect(self.trigger_set)
	...
	def newImage(self, dt):
		self.dt=dt
		if self.bg:
			self.dt-=self.bg_image
		self.ccd.img_cb=dummy

		if self.diff:
			self.trigger_signal.emit(0)
			if self.plot_lockin():
				self.ccd.img_cb=self.newImage
				return			
		...
		if self.pstate==0:
			...
			self.accuImg+=self.dt
			self.accuPry=self.accuImg.mean(axis=0)
			#self.imA.set_array(self.accuImg)
			if self.scaling:
				self.scale_accu()
	...
	def plot_lockin(self):
		...
		if self.pon is None or self.poff is None:
			return True
		self.dt=np.log10(self.pon/self.poff)
		self.pon=None
		self.poff=None
		return False
